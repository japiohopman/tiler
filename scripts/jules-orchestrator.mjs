...
/** Finds only top-level checkbox tasks under a named ### heading inside ## Now. */
...
async function main() {
    ...
    if (!isTaskConfirmedDone(roadmapText, state.activeSession.task)) {
      console.log(
        `PR #${prNumber} is merged, but "${state.activeSession.task}" is still unchecked in ROADMAP.md. ` +
          'Jules should only check the box after its own verification. Manual inspection required.',
      );
      return;
    }
  ...
  if (!state.activeSession) {
    ...
    const prompt = [
      ...
      next.text,
      'Follow AGENT_RULES.md strictly — especially: do not claim something works without verifying it, and stay inside the relevant module.',
      'Start from the main branch.',
      'When done AND personally verified, edit ROADMAP.md yourself and change only this task checkbox from',
      `"- [ ] ${next.text}" to "- [x] ${next.text}" — in place. Do not move or delete the Problem/Goal/Acceptance bullets beneath it.`,
