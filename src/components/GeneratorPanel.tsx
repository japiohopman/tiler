...
const getMaterialIcon = (id: MaterialId) => {
  switch (id) {
    ...
    case 'stone':
      return <Gem className="w-4 h-4 text-slate-300" aria-hidden="true" />;
    case 'brick':
      return <Layers className="w-4 h-4 text-orange-400" aria-hidden="true" />;
    case 'dirt':
      return <Droplets className="w-4 h-4 text-amber-600" aria-hidden="true" />;
    case 'wood':
      return <Mountain className="w-4 h-4 text-amber-500" aria-hidden="true" />;
    case 'metal':
      return <ShieldCheck className="w-4 h-4 text-slate-400" aria-hidden="true" />;
    case 'moss':
      return <Trees className="w-4 h-4 text-emerald-500" aria-hidden="true" />;
    case 'water':
    ...
    <section role="region" aria-label="AI Texture Generator Controls" className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 flex flex-col gap-5 shadow-lg">
      ...
      <div className="space-y-2">
        <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
          <span>Target Material ({TARGET_MATERIALS.length} Presets)</span>
          <span className="text-[11px] text-slate-500 font-normal">2D Ground Surfaces</span>
        </label>
        <div role="radiogroup" aria-label="Target Material" className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
      ...
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          placeholder="e.g., weathered cracks, subtle pebbles, lichen..."
        />
        ...
        <div className="pt-1 flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <span>Compact SDXL Prompt Architecture</span>
          ...
          {showPromptDetails && (
            <div className="mt-1 p-2.5 rounded-lg bg-slate-950/90 border border-slate-800 text-[10px] text-slate-400 space-y-1 leading-relaxed">
              <p className="font-semibold text-slate-300">Compact SDXL Prompt Rules (15-25 words target):</p>
              <ul className="list-disc pl-4 space-y-0.5 text-slate-400">
                <li>Single authoritative promptDescriptor per material</li>
                <li>Zero cross-material contamination or descriptor repetition</li>
                <li>Style descriptor describes art style only (no top-down/orthographic/tileset)</li>
                <li>User modifiers filtered for scene composition terms</li>
                <li>Minimal universal tileability tail: <code className="text-amber-300 font-mono">seamless tileable texture</code></li>
              </ul>
            </div>
          )}
